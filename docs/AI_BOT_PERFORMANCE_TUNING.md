...
공격형 AI가 공격 가능한 공에서도 블로킹이나 대기 판단에 밀려 스파이크를 덜 쓰는 문제를 완화했다.

- 공격형 `spikeMultiplier`와 전용 `spikeFreedom`을 높여 스파이크 가능 거리/높이를 확장했다.
- 공격형은 공격 가능한 네트 근처 공에서 `BLOCK`보다 `SPIKE`를 우선 고려한다.
- 공격형 점프 lead tick과 스파이크 cooldown을 완화해 점프 후 `ACTION` 타이밍을 더 자주 만든다.
- 랠리형/수비형은 기존처럼 블록과 안정 수비 우선순위를 유지한다.

추가 검증:

- 공격형: 네트 근처 높은 공에서 `2P_ACTION` 스파이크 우선 생성
- 랠리형: 같은 네트 위협 상황에서 여전히 `2P_DOUBLE_UP` 블록 생성

## 12. 랠리형 점프 서브 타점 보정

랠리형 AI가 토스 직후 너무 낮은 위치에서 바로 `ACTION`을 눌러 점프 서브 타이밍을 놓치는 문제를 수정했다.

- profile별 `serveHitRatio`를 추가해 토스 최고점 대비 충분히 공이 올라왔을 때만 점프 서브를 친다.
- 랠리형은 `serveHitRatio: 0.64`로 설정해 공격형보다 더 안정적인 높은 타점을 기다린다.
- 공중 상태에서 타점이 아직 낮으면 일반 오버핸드 서브로 빠지지 않고 기다린다.
- 타이밍을 놓쳐 공이 떨어지는 경우에만 기존 fallback 서브를 허용한다.

추가 검증:

- 랠리형: 낮은 초기 공중 타점에서는 `ACTION`을 누르지 않음
- 랠리형: GameLoop 시뮬레이션에서 충분히 높은 타점까지 기다렸다가 `JUMP_SERVE_HIT`으로 rally 전환
